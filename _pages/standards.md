---
layout: single
title: "Standards"
permalink: /standards/
author_profile: true
---

Standards, guidelines and product category rules for the environmental assessments of photovoltaic systems.

## LCA Standards

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://www.iso.org/standard/76121.html" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">ISO 14040</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Environmental Management - Life Cycle Assessment - Principles and Framework</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">ISO 14040:2006/Amd 1:2020 | 2020-09</p>
    </a>
  </div>
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://www.iso.org/standard/76122.html" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">ISO 14044</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Environmental Management - Life Cycle Assessment - Requirements and Guidelines</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">ISO 14044:2006/Amd 2:2020 | 2020-09</p>
    </a>
  </div>
</div>

## EPD Standards

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://www.dinmedia.de/en/standard/din-en-15804/344735627" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">EN 15804</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Sustainability of construction works - Environmental product declarations - Core rules for the product category of construction products</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">EN 15804:2012+A2:2019+AC:2021 | 2022-03</p>
    </a>
  </div>
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://www.iso.org/standard/61694.html" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">ISO 21930</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Sustainability in buildings and civil engineering works - Core rules for environmental product declarations of construction products and services</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">ISO 21930:2017 | 2017-07</p>
    </a>
  </div>
</div>

## PCF Standards

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://www.iso.org/standard/71206.html" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">ISO 14067</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Greenhouse gases - Carbon footprint of products - Requirements and guidelines for quantification</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">ISO 14067:2018 | 2018-08</p>
    </a>
  </div>
</div>

## EPD Product Category Rules (PCR)

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://environdec.com/pcr-library/pcr_d2322eee-828a-4995-31a7-08da281808ad" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-file-alt"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">PCR 2019:14</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">International EPD System - Construction products</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">IES PCR 2019:14 | 2025-06 (v2.0.1)</p>
    </a>
  </div>
</div>

## EPD Specific Product Category Rules for PV

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://environdec.com/pcr-library/pcr_d2322eee-828a-4995-31a7-08da281808ad" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-solar-panel"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">IES c-PCR-016</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">International EPD System - Photovoltaic modules and parts thereof</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">EPD | 2022-04 | v1.2</p>
    </a>
  </div>
</div>

## LCA Guidelines for PV

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://iea-pvps.org/key-topics/methodology-guidelines-on-life-cycle-assessment-of-photovoltaic-2020/" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-solar-panel"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">IEA-PVPS LCA-2020</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Methodology Guidelines on Life Cycle Assessment of Photovoltaic 2020</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">LCA | 2020-04 | 4th Edition</p>
    </a>
  </div>
</div>

## PCF Guidelines for PV

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-bottom: 30px;">
  <div style="text-align: center; padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafced; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 4px 8px rgba(0,0,0,0.1)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
    <a href="https://iea-pvps.org/key-topics/methodology-guidelines-on-life-cycle-assessment-of-photovoltaic-2020/" style="text-decoration: none; color: inherit; display: block;">
      <div style="font-size: 48px; margin-bottom: 10px; color: #3c8062;">
        <i class="fas fa-solar-panel"></i>
      </div>
      <h3 style="margin: 0; font-size: 18px; color: #3c8062;">IEA-PVPS LCA-2020</h3>
      <p style="margin: 5px 0 0 0; font-size: 14px; color: #555;">Methodology Guidelines on Life Cycle Assessment of Photovoltaic 2020</p>
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #888;">LCA | 2020-04 | 4th Edition</p>
    </a>
  </div>
</div>
